...
class TestDraftSkTool(unittest.TestCase):
    ...
    def _stub_modules(
        ...
        *,
        ...
        soffice_pdf=None,       # render_docx_to_pdf (soffice) return / None
        ...
        async def _render_docx_to_pdf(docx_bytes_in):
            calls["soffice"] = {"docx_bytes": docx_bytes_in}
            return soffice_pdf
        ...
        _patch(st_mod, "render_docx_to_pdf", _render_docx_to_pdf)
        return calls
    ...
    def test_renders_and_queues_only_pdf_with_filled_and_blank_report(self):
        # 3 fields filled from real sources, 2 blank → ONLY the PDF is queued
        # (the officer wants a single readable file). With no soffice PDF, it
        # falls back to the fpdf2 view. The note reports both field sets.
        ...
        # ONLY the PDF is queued — no .docx.
        self.assertEqual(len(queue), 1)
        q = queue[0]
        self.assertEqual(q["filename"], "Rekomtek_000123456.pdf")
        self.assertEqual(q["mime_type"], "application/pdf")
        self.assertNotIn(".docx", " ".join(x["filename"] for x in queue))
    ...
    def test_soffice_pdf_preferred_over_fpdf2(self):
        # When LibreOffice yields a PDF, use it (faithful letterhead) and do NOT
        # fall back to the fpdf2 text view.
        out, queue = self._run_draft(
            sk_ctx=self._sk_ctx(),
            data={"nama_pemohon": "CASMO"}, classes={"nama_pemohon": "profile"},
            soffice_pdf=b"%PDF-soffice-real")
        self.assertEqual(len(queue), 1)
        self.assertEqual(queue[0]["mime_type"], "application/pdf")
        self.assertEqual(queue[0]["content"], b"%PDF-soffice-real")

    def test_docx_last_resort_when_no_pdf_possible(self):
        # If NEITHER soffice nor fpdf2 can produce a PDF, send the .docx so the
        # officer still gets the draft.
        out, queue = self._run_draft(
            sk_ctx=self._sk_ctx(),
            data={"nama_pemohon": "CASMO"}, classes={"nama_pemohon": "profile"},
            soffice_pdf=None, pdf_view=None)
        self.assertEqual(len(queue), 1)
...
class TestSystemPromptContent(unittest.TestCase):
    ...
    def test_officer_prompt_draft_means_sk_and_sk_not_in_siap(self):
        ...
        self.assertIn("PDF", p)  # draft is delivered as a PDF attachment
